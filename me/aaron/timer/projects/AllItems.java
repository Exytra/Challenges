...
public class AllItems {
    ...
    public static void next() {
        ...
        if (items.size() != 0) {
            item = Material.valueOf(items.get(Utils.getRandomInt(0, items.size() - 1)));
            for (Player pl : Bukkit.getOnlinePlayers()) {
                pl.sendMessage("§lAlle Items §8» §7Folgendes Item wurde registriert: §e§l" + Utils.firstLatterCapitalized(old.getKey().getKey().replace("_", " ")) + "§7. Neues Item: §a§l" + Utils.firstLatterCapitalized(item.getKey().getKey().replace("_", " ")) + " §8(§7Es fehlen noch §f§l" + (items.size()) + " §7Items§8)");
            }
        }
    }
}
