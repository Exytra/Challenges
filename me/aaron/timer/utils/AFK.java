                    ...
                    if (MoveListener.lastMovement.get(pl) < System.currentTimeMillis() / 1000 - 300) {
                        if (!afk.contains(pl)) {
                            ...
                            for (Player pl2 : Bukkit.getOnlinePlayers()) {
                                if (pl2 == pl) {
                                    pl.sendMessage(Main.getPrefix("AFK", "Du bist nun AFK."));
                                } else {
                                    pl2.sendMessage(Main.getPrefix("AFK", "Der Spieler §9" + pl.getName() + " §7ist nun AFK."));
                                }
                            }
                        }
                    } else {
                        if (afk.contains(pl)) {
                            ...
                            for (Player pl2 : Bukkit.getOnlinePlayers()) {
                                if (pl2 == pl) {
                                    pl.sendMessage(Main.getPrefix("AFK", "Du bist nun nicht mehr AFK."));
                                    pl.sendMessage(Main.getPrefix("AFK", "Du warst AFK für " + Timer.ConvertTimerTime(Integer.parseInt("" + ((System.currentTimeMillis() / 1000 - 300) - oldTime.get(pl))), "§9§l") + " §7AFK."));
                                } else {
                                    pl2.sendMessage(Main.getPrefix("AFK", "Der Spieler §9" + pl.getName() + " §7ist nun nicht mehr AFK."));
                                }
                            }
                        }
                    }
