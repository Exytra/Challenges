                ...
                if (Timer.state == Timer.TimerState.RUNNING || SettingsModes.settings.get(SettingsItems.ItemType.TIMER) == SettingsItems.ItemState.DISABLED) {
                    if (!(currenttime >= neededtime)) {
                        if (forcedbiome == null) {
                            ...
                            bossBar.setTitle("Warten auf neues §9Biom§7 ...");
                        ...
                        if (forcedbiome == null) {
                            ...
                            for (Player pl : Bukkit.getOnlinePlayers()) {
                                pl.sendMessage(Main.getPrefix("Force Biome", "§9Finde §9" +  Utils.firstLatterCapitalized(forcedbiome.toString().replace("_", " ")) + " §7in §9" + Timer.ConvertTimerTime(neededtime, "§9")));
                            }
                        } else {
                            for (Player pl : Bukkit.getOnlinePlayers()) {
                                pl.setHealth(0);
